...
router.post(`/logout`, ck.auth.authRole(), async (req, res) => {
  const result = await ck.userCtr.logout(req.reqUser)
  res.suc(result)
})

router.post(`/updateById`, ck.auth.authRole(), async (req, res) => {
...
router.get(`/getById`, ck.auth.authRole(`staff`, `manager`, `admin`), async (req, res) => {
...
router.get(`/getList`, ck.auth.authRole(`staff`, `manager`, `admin`), async (req, res) => {
